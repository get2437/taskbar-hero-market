...
export function AdminPanel() {
  ...
  async function action(kind: "refresh" | "reanalyze" | "cache") {
    ...
    try {
      let res: Response;
      if (kind === "cache") {
        res = await fetch("/api/admin/cache", { method: "DELETE", headers: { "x-admin-token": token } });
      } else {
        const fetchParam = kind === "reanalyze" ? "?fetch=false" : "";
        res = await fetch(`/api/admin/refresh${fetchParam}`, { method: "POST", headers: { "x-admin-token": token } });
      }
      const data = await res.json();
      if (!res.ok) {
        setMsg({ ok: false, text: data.error ?? "失敗しました" });
      } else if (kind === "cache") {
        setMsg({ ok: true, text: `キャッシュを削除しました (${data.removed} keys)` });
      } else {
        setMsg({
          ok: true,
          text: `完了: 取得${data.fetched} / 分析${data.analyzed} / 異常${data.anomalies} / 通知${data.notified}${data.skippedFetch ? " (取得スキップ)" : ""}`,
        });
      }
      loadStatus();
    } catch (e) {
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            <Button onClick={() => action("refresh")} disabled={busy != null}>
              {busy === "refresh" ? <Loader2 className="h-4 w-4 animate-spin" /> : <RefreshCw className="h-4 w-4" />}
              データ更新 (取得+分析)
            </Button>
            <Button variant="secondary" onClick={() => action("reanalyze")} disabled={busy != null}>
              {busy === "reanalyze" ? <Loader2 className="h-4 w-4 animate-spin" /> : <Activity className="h-4 w-4" />}
              再分析のみ
            </Button>
            <Button variant="outline" onClick={() => action("cache")} disabled={busy != null}>
